...
def follow_like_comment(number):
    ...
    for i in range (len(profiles_list)): # 2 pages
        ...
        # first media element
        media_element = driver.find_element(By.XPATH,'//div[@class="eLAPa"]')
        media_element.click()
        time.sleep(2)

        for k in range (number):
            # like
            like = driver.find_element(By.XPATH,'//button[@class="wpO6b "]//*[@aria-label="Like"]')
            like.click()
            time.sleep(1)
            # comment
            comment_1 = driver.find_element(By.XPATH,'//div[@class="RxpZH"]')
            comment_1.click()
            time.sleep(1)

            comment_2 = driver.find_element(By.CLASS_NAME,'Ypffh')
            comment_2.send_keys(random.choice(comments))
            time.sleep(1)
            # post
            post = driver.find_element(By.XPATH,'//button[text()="Post"]')
            post.click()
            time.sleep(1)
